# Capillary UI style lab

This public, non-publishable workspace application is Capillary UI's deterministic CSS
...
```sh
pnpm --filter @capillaryjs/capillary-ui-demo dev
```

The page loads exactly four Capillary UI styling inputs in this order: an external
...
Capillary UI stylesheet. `style-lab.css` is separate application-owned page layout.

Vite resolves Capillary, Capillary UI, and Capillary Viz to their package source entry
...
Meridian explicitly places both `cap-fill-horizontal` and
`cap-fill-vertical` on its application root. It therefore occupies exactly
...
Only approved or currently reviewed public Capillary UI types are instantiated. The
...
reusable presentation behavior belongs in Capillary UI or Capillary Viz.
