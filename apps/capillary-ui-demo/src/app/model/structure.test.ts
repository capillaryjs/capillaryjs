...
test('Meridian renders one flat island layer around its routed workspace', () => {
    ...
    assert.ok(document.querySelector('.meridian-navigation > cap-sidebar.meridian-scope.island'))
    assert.ok(document.querySelector('.meridian-navigation > cap-panel.meridian-view.island'))
    assert.ok(document.querySelector('body > main > cap-panel.demo-harness.island'))
    const scopeLabels = [...document.querySelectorAll<HTMLElement>(
        '.scope-tree [role="treeitem"] > cap-label',
    )]
    ...
    const split = document.querySelector('cap-splitview.register-results-split')
    assert.ok(split)
    assert.equal(split.classList.contains('island'), false)
    assert.ok(split.querySelector(':scope > cap-primary > cap-panel.island'))
    assert.ok(split.querySelector(':scope > cap-secondary > cap-panel.island'))
