...
describe('action and text controls', () => {
    test('Label keeps native association while supporting live and rich content', () => {
        ...
        assert.equal(label.dataset.capComponent, 'label')
        ...
        assert.equal(requiredAt(document.querySelectorAll<HTMLLabelElement>('label'), 1).innerHTML,
            '<strong data-cap="">A deliberately long native-label description</strong>')
    })
    ...
    test('Button uses a fixed host around its native, disableable action', () => {
        ...
        const button = Button.new({label: 'Save', onClick: () => calls += 1})
            .attachTo(document.body)
        const host = requiredQuery<HTMLElement>('cap-button')
    ...
    test('Button busy state preserves its native node and blocks activation', () => {
        ...
        assert.equal(element.getAttribute('aria-describedby'),
            requiredQuery<HTMLElement>('cap-error[role="alert"]', host).id)
        assert.equal(requiredQuery('cap-erroricon', host).getAttribute('aria-hidden'), 'true')
        assert.equal(requiredQuery('cap-errortext', host).textContent, 'Refresh failed')
    ...
    test('Toolbar supplies role, name, orientation, and children', () => {
        ...
        assert.equal(toolbar.localName, 'cap-toolbar')
    ...
    test('Textbox supports internal state, callbacks, labels, and errors', () => {
        ...
        assert.equal(input.parentElement?.localName, 'cap-textbox')
...
describe('choice controls', () => {
    test('theme and color pickers update independent stylesheet links', () => {
        ...
        assert.ok([...document.querySelectorAll('.cap-theme-picker, .cap-colors-picker')]
            .every((picker) => picker.querySelector(':scope > cap-selectshell > select') != null))
        assert.ok([...document.querySelectorAll('.cap-theme-picker, .cap-colors-picker')]
        ...
        assert.equal(document.head.querySelectorAll('link[data-cap-stylesheet]').length, 2)
        ...
        assert.equal(
            document.head.querySelector<HTMLLinkElement>(
                'link[data-cap-stylesheet="theme"]',
            )?.dataset.capSelection,
            'shiny',
        )
    })
    ...
    test('Dropdown tracks options, typed values, and external updates', () => {
        ...
        assert.equal(select.parentElement?.localName, 'cap-selectshell')
        assert.equal(select.parentElement?.parentElement?.localName, 'cap-dropdown')
    ...
    test('Dropdown uses its native select for live availability and validation', () => {
        ...
        const host = requiredQuery<HTMLElement>('cap-dropdown')
    ...
    test('Dropdown derives busy and error feedback from a caller-owned options source', () => {
        ...
        assert.equal(select.getAttribute('aria-invalid'), 'true')
        assert.equal(requiredQuery('cap-error[role="alert"]').textContent, 'Options unavailable')

        options.setWithState([{value: 'api', label: 'API'}], FetchState.Error, null)
        assert.equal(requiredQuery('cap-error[role="alert"]').textContent,
    ...
    test('Toggle is a keyboard-operable radio group', () => {
        ...
        const host = requiredQuery<HTMLElement>('cap-toggle')
        const label = requiredQuery<HTMLElement>('cap-label', host)
        const radiogroup = requiredQuery<HTMLElement>('cap-options[role="radiogroup"]', host)
        assert.equal(host.dataset.capComponent, 'toggle')
        assert.equal(host.id.startsWith('cap-toggle-'), true)
    ...
    test('Toggle follows live availability and validation state through native semantics', () => {
        ...
        assert.equal(requiredQuery<HTMLElement>('cap-error[role="alert"]', host).textContent,
            'Choose a status')
    })
    ...
    test('RadioGroup renders native grouped radio inputs', () => {
        ...
        assert.equal(requiredQuery('cap-radiogroup').dataset.capComponent, 'radio-group')
        ...
        assert.equal(requiredQuery('cap-radiobutton', fieldset).localName, 'cap-radiobutton')
    ...
    test('RadioGroup binds only its declared live boolean props', () => {
        ...
        const host = requiredQuery<HTMLElement>('cap-radiogroup')
        const fieldset = requiredQuery<HTMLFieldSetElement>('fieldset')
    ...
    test('an owner can explicitly rerender RadioGroup with changing options', () => {
        ...
        assert.deepEqual(
            [...document.querySelectorAll('cap-radiobutton')]
    ...
    test('RadioButton exposes a native standalone input', () => {
        ...
        const host = requiredQuery<HTMLElement>('cap-radiobutton')

        assert.equal(input.name, 'setting')
        assert.equal(input.value, 'enabled')
        assert.equal(input.parentElement?.parentElement?.localName, 'cap-radiobutton')
        assert.equal(input.nextElementSibling?.localName, 'cap-checkshell')
    ...
    test('Checkbox variants expose semantic state and keyboard cycling', () => {
        ...
        assert.equal(control.closest('cap-checkbox'), host)
        assert.equal(control.nextElementSibling?.localName, 'cap-checkshell')
        ...
        assert.equal(requiredQuery('cap-quadcheckbox').dataset.capComponent, 'quadcheckbox')
...
describe('layout controls', () => {
    test('Placeholder renders one hidden fixed host with a bounded width', () => {
        ...
        const lowerBound = requiredQuery<HTMLElement>('cap-placeholder')
        ...
        assert.equal(requiredQuery<HTMLElement>('cap-placeholder').style.width, '100%')

        document.body.replaceChildren()
        Placeholder.new().attachTo(document.body)
        assert.equal(requiredQuery<HTMLElement>('cap-placeholder').style.width, '65%')
        assert.throws(
    ...
    test('DescriptionList renders native term/value semantics', () => {
        ...
        const host = requiredQuery('cap-descriptionlist')
        const list = requiredQuery<HTMLDListElement>('dl', host)
        assert.equal(host.querySelector('div, [data-cap-component]'), null)
    ...
    test('InfoPanel without title omits header and region role', () => {
        ...
        assert.equal(host.querySelector('cap-header'), null)
        const list = requiredQuery<HTMLDListElement>('dl', host)
    ...
    test('Breadcrumb renders an ordered path with the last item current', () => {
        ...
        const host = requiredQuery('cap-breadcrumb')
    ...
    test('Layout owns generic direction, allocation, and explicit scrolling', () => {
        ...
        const layout = requiredQuery<HTMLElement>('cap-layout')
        assert.equal(
            layout.className,
            'cap-layout-horizontal cap-size-flexible cap-scroll',
        )
    ...
    test('Layout and Panel render presentation-context markers', () => {
        Layout.new({horizontal: true, context: 'form', children: 'Form'})
            .attachTo(document.body)
        assert.equal(
            requiredQuery<HTMLElement>('cap-layout').getAttribute('data-cap-context'),
            'form',
        )

        Panel.new({header: 'Details', context: 'control', children: 'Body'})
            .attachTo(document.body)
        const panel = requiredQuery<HTMLElement>('cap-panel')
        assert.equal(panel.getAttribute('data-cap-context'), 'control')
        assert.equal(
            requiredQuery<HTMLElement>('cap-layout', panel)
                .getAttribute('data-cap-context'),
            null,
        )
    })
    ...
    test('SplitView owns two Layout panes, an accessible separator, and validation', () => {
        ...
        const split = requiredQuery<HTMLElement>('cap-splitview')
        assert.equal(split.className, 'horizontal')
        assert.equal(split.style.getPropertyValue('--split-primary-size'), '18rem')
        assert.equal(requiredQuery('cap-primary', split).textContent, 'Tree')
        assert.equal(
            requiredQuery('cap-secondary', split).getAttribute('aria-label'),
            'Project details',
        )
        assert.equal(requiredQuery('cap-primary', split).getAttribute('role'), 'region')
        assert.equal(requiredQuery<HTMLElement>('cap-primary', split).tabIndex, 0)
        assert.equal(requiredQuery<HTMLElement>('cap-secondary', split).tabIndex, 0)
        assert.equal(split.querySelector('div'), null)
        assert.match(requiredQuery('cap-primary', split).className, /cap-layout-vertical/)
        const separator = requiredQuery<HTMLElement>('cap-separator', split)
        ...
        const primary = requiredQuery<HTMLElement>('cap-primary', split)
        ...
        const vertical = requiredQuery<HTMLElement>('cap-splitview.vertical')
        assert.equal(vertical.style.getPropertyValue('--split-primary-size'), '45%')
        assert.equal(requiredQuery('cap-primary', vertical).textContent, 'Navigation')
        assert.equal(requiredQuery('cap-secondary', vertical).textContent, 'Details from children')
        assert.equal(requiredQuery('cap-primary', vertical).hasAttribute('role'), false)
        assert.equal(requiredQuery('cap-secondary', vertical).hasAttribute('aria-label'), false)
    ...
    test('ProgressBar tracks an emitter in determinate and indeterminate modes', () => {
        ...
        const host = requiredQuery<HTMLElement>('cap-progressbar')
        assert.equal(host.querySelector('div'), null)
        assert.equal(requiredQuery<HTMLLabelElement>('label', host).htmlFor, progress.id)
        assert.equal(requiredQuery('cap-content', host).getAttribute('aria-hidden'), 'true')
        assert.equal(requiredQuery('cap-label', host).textContent, 'Project refresh')
        assert.equal(requiredQuery('cap-inverse', host).textContent, 'Project refresh')
        ...
        value.setWithState(null, FetchState.Loading)
        ...
        const visualProgress = requiredQuery<HTMLElement>('cap-progress', host)
        ...
        const richHost = requiredQuery<HTMLElement>('cap-progressbar')
        assert.equal(requiredQuery('label > strong', richHost).textContent, 'Rich refresh label')
        assert.equal(requiredQuery('cap-label > strong', richHost).textContent, 'Rich refresh label')
        assert.equal(requiredQuery('cap-inverse > strong', richHost).textContent, 'Rich refresh label')
    })
    ...
    test('Header renders native heading levels inside its component host', () => {
        ...
        const header = requiredQuery<HTMLElement>('cap-header')
        ...
        assert.equal(requiredQuery<HTMLHeadingElement>('cap-header h6').textContent,
            'Deep heading')
    ...
    test('GroupBox labels a bordered group through a vertical Header', () => {
        ...
        const group = requiredQuery<HTMLElement>('cap-groupbox')
        const header = requiredQuery<HTMLElement>(':scope > cap-header', group)
        const title = requiredQuery<HTMLHeadingElement>(':scope > h2', header)
        const content = requiredQuery<HTMLElement>(':scope > cap-content', group)
    ...
    test('Panel uses a labelled component host and explicit orientation', () => {
        ...
        const section = requiredQuery<HTMLElement>('cap-panel')
        const header = requiredQuery<HTMLElement>('cap-header', section)
        const title = requiredQuery<HTMLElement>('h2', header)
        const content = requiredQuery<HTMLElement>('cap-layout', section)
        ...
        assert.equal(
            content.className,
            'panel-content cap-layout-horizontal cap-scroll',
        )
        assert.equal(content.textContent, 'Details')
        assert.equal(requiredQuery(':scope > cap-button', section).textContent, 'Save')
    ...
    test('OptionGroup uses a named marker for trailing legend content', () => {
        ...
        const group = requiredQuery('cap-optiongroup')
        ...
        assert.equal(requiredQuery('cap-error[role="alert"]', group).textContent,
            'Select a severity')
        assert.throws(
    ...
    test('shell components map explicit allocation to their intentional hosts', () => {
        ...
        assert.equal(requiredQuery<HTMLElement>('cap-panel').className, 'cap-size-flexible')
        assert.equal(
            requiredQuery<HTMLElement>('cap-panel > cap-layout').className,
            'panel-content cap-layout-vertical cap-scroll',
        )
        panel.destroy()

        const sidebar = Sidebar.new({allocation: 'natural', children: 'Natural'})
        sidebar.attachTo(document.body)
        assert.equal(requiredQuery<HTMLElement>('cap-sidebar').className, 'cap-size-natural')
        sidebar.destroy()
    ...
    test('fixed component hosts opt into island treatment explicitly', () => {
        ...
        const host = requiredQuery<HTMLElement>('cap-panel')
    ...
    test('TabPanel wires tab semantics, content, clicks, and arrow keys', () => {
        ...
        assert.equal(requiredQuery('cap-tabpanel').dataset.capComponent, 'tab-panel')
        assert.equal(requiredQuery('cap-tabline').dataset.capComponent, 'tab-line')
    ...
    test('nested components use fixed Capillary UI host names', () => {
        const runtime = createCapillaryUiRuntime()
        const panel = runtime.create(Panel, {
            children: [h(Textbox, {label: 'Name'})],
        })
        runtime.mount(panel, document.body)
        assert.ok(document.querySelector('cap-panel'))
        assert.ok(document.querySelector('cap-textbox'))
        assert.ok(document.querySelector('cap-panel[data-cap]'))
        panel.destroy()
    })
})
...
describe('data components', () => {
    test('ListView re-renders when its plain-array items prop changes', () => {
        ...
        assert.ok(document.querySelector('cap-listview [role="status"]'))
        list.destroy()
    })
})
