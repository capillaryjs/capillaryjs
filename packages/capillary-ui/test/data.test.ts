...
test('initial skeletons bypass renderers while refreshes retain collection content', () => {
    ...
    items.setWithState(values, FetchState.Loading)
    assert.ok(renders > rendered, 'retained rows are rendered during a refresh')
    assert.match(document.body.textContent ?? '', /Cached row/)
    assert.match(document.body.textContent ?? '', /Cached child/)
    ...
    const refreshedRenders = renders
    items.setWithState(values, FetchState.Initial)
    assert.equal(renders, refreshedRenders, 'initial loading does not invoke application renderers')
...
describe('stable data components', () => {
    ...
    test('TreeView supports controlled expansion, selection, navigation, and cleanup', () => {
        ...
        assert.equal(requiredQuery('cap-treeview').dataset.capComponent, 'tree-view')
        assert.ok(!requiredQuery('cap-treeview').classList.contains('datacomponentlike'))
        assert.ok(requiredQuery('cap-treeview > ul[role="tree"]'))
        assert.equal(document.querySelector('cap-treeview div'), null)
        ...
        assert.ok(first.querySelector(':scope > cap-expander'))
        const firstLabel = requiredQuery<HTMLElement>(':scope > cap-label', first)
        ...
        const childLabel = requiredQuery<HTMLElement>(':scope > cap-label', child)
    ...
    test('TreeView keeps retained rows visible during loading and shows errors', () => {
        ...
        const placeholderList = requiredQuery('cap-treeview > ul[aria-hidden="true"]')
        assert.equal(placeholderList.querySelectorAll('cap-placeholder').length, 3)
        ...
        assert.equal(requiredQuery('cap-error[role="alert"]').textContent, 'Tree unavailable')
    ...
    test('Dialog synchronizes native modality, cancel, and focus restoration', () => {
        ...
        open.set(true)
        const host = requiredQuery<HTMLElement>('cap-dialog')
        const element = requiredQuery<HTMLDialogElement>('dialog')
        assert.equal(host.dataset.capComponent, 'dialog')
        assert.equal(element.parentElement, host)
        assert.ok(element.querySelector('cap-content'))
    ...
    test('ListView shares its selected emitter and preserves key selection on refresh', () => {
        ...
        const host = requiredQuery('cap-listview')
        assert.equal(host.querySelector('div, [data-part]'), null)
        assert.equal(host.querySelector('[data-cap-selectable-row]')?.localName, 'li')
        requiredAt(rows, 1).dispatchEvent(new MouseEvent('click', {bubbles: true}))
    ...
    test('keeps loading, empty, and error messages outside listbox semantics', () => {
        ...
        assert.equal(requiredQuery('cap-listview').querySelector('div, [data-part]'), null)
    ...
    test('DataTable direct data sorts, filters, selects only body rows, and stays semantic', () => {
        ...
        assert.equal(requiredQuery('cap-datatable').dataset.capComponent, 'data-table')
        assert.equal(requiredQuery('cap-datatable').classList.contains('datacomponentlike'), false)
        assert.equal(requiredQuery('thead').hasAttribute('data-cap-component'), false)
    ...
    test('renders loading, partial, empty, error, and retry states separately', () => {
        ...
        assert.equal(requiredQuery('cap-placeholder').dataset.capComponent, 'placeholder')
        ...
        assert.equal(requiredQuery('[role="alert"]').localName, 'cap-error')
        assert.equal(requiredQuery('[role="alert"] cap-erroricon')
            .getAttribute('aria-hidden'), 'true')
        assert.equal(requiredQuery('[role="alert"]').parentElement?.localName, 'cap-datatable')
        assert.equal(requiredQuery('button').parentElement?.localName, 'cap-datatable')
    ...
    test('FilterPanel reports caller-owned filter state through native group content', async () => {
        ...
        const host = requiredQuery<HTMLElement>('cap-filterpanel')
        assert.equal(host.dataset.capComponent, 'filter-panel')
    ...
    test('FilterPanel exposes loading and error messages without losing retained choices', () => {
        ...
        assert.equal(requiredQuery('cap-filterpanel').getAttribute('aria-busy'), 'true')

        options.setWithState(['Ops'], FetchState.Loading)
        assert.equal(requiredQuery<HTMLElement>('cap-filterpanel').getAttribute('aria-busy'), 'true')
    ...
    test('remote table serializer keeps table encoding outside Capillary', () => {
