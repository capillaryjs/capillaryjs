...
describe('style registry', () => {
    ...
    test('collects one semantic loading/error system across controls and data views', () => {
        ...
        assert.match(stylesheet, /cap-progressbar:has\(> progress\[aria-busy="true"\]\) > cap-content::after[\s\S]*animation:\s*cap-working-progress/)
        assert.match(stylesheet, /cap-button > button:disabled:not\(\[aria-busy="true"\]\)/)
        assert.match(stylesheet, /cap-button > button\[aria-busy="true"\][^}]*background:\s*var\(--working-background-image\), var\(--button-background\)/)
    ...
    test('collects the native DataTable family through the fixed table boundary', () => {
        ...
        assert.match(stylesheet, /cap-datatable > table\s*\{[^}]*border-collapse:\s*separate[^}]*border-spacing:\s*0/)
        assert.match(stylesheet, /cap-datatable > table > thead\s*\{[^}]*color:\s*var\(--table-header-color\)[^}]*background:\s*var\(--table-header-background\)/)
        assert.match(stylesheet, /cap-datatable > table > thead > tr > th\s*\{[^}]*position:\s*sticky[^}]*inset-block-start:\s*0[^}]*height:\s*1\.5rem[^}]*background:\s*var\(--table-header-background\)/)
        assert.match(stylesheet, /cap-datatable > table > thead > tr > th\[aria-sort\]\s*\{[^}]*position:\s*sticky[^}]*inset-block-start:\s*0/)
        assert.match(stylesheet, /button\.sort > span\.sortindicator\s*\{[^}]*right:\s*20px[^}]*height:\s*1\.25em[^}]*text-align:\s*center/)
        assert.match(stylesheet, /th\[aria-sort="none"\] > button\.sort > span\.sortindicator\s*\{[^}]*opacity:\s*0\.5/)
        assert.match(stylesheet, /span\.sortindicator::before,[\s\S]*span\.sortindicator::after\s*\{[^}]*border-left:\s*\.36em solid transparent[^}]*border-right:\s*\.36em solid transparent/)
        assert.match(stylesheet, /span\.sortindicator::before\s*\{[^}]*border-bottom:\s*\.5em solid currentColor/)
        assert.match(stylesheet, /span\.sortindicator::after\s*\{[^}]*border-top:\s*\.5em solid currentColor/)
        assert.match(stylesheet, /button\.filter\s*\{[^}]*right:\s*2px[^}]*opacity:\s*0\.5/)
        assert.match(stylesheet, /button\.filter::before\s*\{[^}]*background:\s*currentColor[^}]*clip-path:\s*polygon\(/)
    ...
    test('collects ProgressBar through its fixed clipped-label parts and native semantics', () => {
        ...
        assert.match(stylesheet, /cap-progressbar\s*\{[^}]*display:\s*flex[^}]*align-items:\s*center[^}]*width:\s*100%[^}]*min-width:\s*0/)
        ...
        assert.match(stylesheet, /cap-progressbar:has\(> progress:indeterminate\[aria-busy="true"\]\) > cap-content > cap-progress\s*\{[^}]*display:\s*none/)
...
describe('four-file styling contract', () => {
    ...
    test('Shiny preserves its chrome, light navigation strip, and glossy graphical tokens', async () => {
        ...
        assert.match(css, /--table-header-background:\s*var\(--ui-gradient\)/)
        assert.match(css, /--table-header-color:\s*var\(--ui-text-color\)/)
