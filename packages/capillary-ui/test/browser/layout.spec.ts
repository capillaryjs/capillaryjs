...
test('natural and flexible allocation works on both axes with cross-axis stretch', async ({page}) => {
    ...
    await page.evaluate((markup) => { document.body.innerHTML = markup }, `
        <div class="horizontal cap-layout-horizontal">
            <div class="natural cap-size-natural"></div>
            <div class="flexible cap-size-flexible"></div>
        </div>
        <div class="vertical cap-layout-vertical">
            <div class="top cap-size-natural"></div>
            <div class="middle cap-size-flexible"></div>
            <div class="bottom cap-size-natural"></div>
        </div>
    `)
...
test('equivalent flexible siblings share space and later app CSS can change the ratio', async ({page}) => {
    ...
    `})
    await page.evaluate((markup) => { document.body.innerHTML = markup }, `
        <div class="row equal cap-layout-horizontal">
            <div class="cap-size-flexible"></div><div class="cap-size-flexible"></div>
        </div>
        <div class="row ratio cap-layout-horizontal">
            <div class="cap-size-flexible"></div><div class="cap-size-flexible"></div>
        </div>
    `)
...
test('nested layouts stay bounded and only the explicit inner owner has scroll range', async ({page}) => {
    ...
    `})
    await page.evaluate((markup) => { document.body.innerHTML = markup }, `
        <main class="cap-fill-horizontal cap-fill-vertical cap-layout-vertical">
            <div class="toolbar cap-size-natural"></div>
            <div class="body cap-size-flexible cap-layout-horizontal">
                <aside class="island sidebar cap-size-natural"></aside>
                <section class="island workspace cap-size-flexible cap-layout-vertical">
                    <div class="content cap-size-flexible cap-scroll" tabindex="0"
...
test('flexible allocation alone does not opt into scrolling', async ({page}) => {
    ...
    await page.evaluate((markup) => { document.body.innerHTML = markup }, `
        <div class="shell cap-layout-vertical">
            <div class="region cap-size-flexible"><div class="large"></div></div>
        </div>
    `)
