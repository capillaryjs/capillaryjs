...
const modernThemePaths = ['capillary', 'shiny', 'soft', 'white', 'minimal'].map((name) => ({
    name,
...
test('Panel exposes its labelled region and live disabled state',
    async ({page}) => {
        ...
        await page.evaluate(() => globalThis.capillaryUiTest.setPanelReviewDisabled(true))
...
test('record-view primitives retain semantics, keyboard behavior, and dialog focus',
    async ({page}) => {
        ...
        await page.evaluate(() => globalThis.capillaryUiTest.setProgress(2))
        ...
        await page.evaluate(() => globalThis.capillaryUiTest.setProgress(null))
        const {violations} = await new AxeBuilder({page})
...
test('History routing restores an outlet, preserves native links, and cleans up', async ({page}) => {
    ...
    await expect(page.getByText('First page')).toHaveCount(0)
    await expect(page).toHaveURL(/\/browser-second\?routing=true$/)
    expect(await page.evaluate(() => globalThis.capillaryUiTest.destroyRouting())).toBe(0)
})
...
test('keeps 1,000-row stable table operations inside the documented budget',
    async ({page, browserName}) => {
        const metrics = await page.evaluate(() => globalThis.capillaryUiTest.measureDataTable(1_000))
...
test('preserves browser-managed input state and replaces event handlers', async ({page}) => {
    ...
    expect(await input.evaluate((element) => ({
        ...
        preserved: globalThis.capillaryUiTest.inputNodePreserved,
    }))).toEqual({start: 6, end: 6, preserved: true})

    await page.evaluate(() => {
        for (let revision = 1; revision <= 5; revision += 1) {
            globalThis.capillaryUiTest.setRevision(revision)
        }
    })
    await page.locator('#event-action').click()
    expect(await page.evaluate(() => globalThis.capillaryUiTest.clicks)).toBe(1)
})

test('patches properties and keyed children without replacing nodes', async ({page}) => {
    await page.evaluate(() => {
        globalThis.capillaryUiTest.setBoolean(false)
        globalThis.capillaryUiTest.reorder(['c', 'a', 'b'])
    })
    ...
    expect(await page.evaluate(() => globalThis.capillaryUiTest.keyedNodesPreserved())).toBe(true)
})

test('destroys removed children once and supports all documented vnode forms', async ({page}) => {
    expect(await page.evaluate(() => ({
        counts: globalThis.capillaryUiTest.lifecycleCounts,
        subscribers: globalThis.capillaryUiTest.childSubscribers,
    }))).toEqual({counts: {initialize: 1, destroy: 0}, subscribers: 1})

    await page.evaluate(() => globalThis.capillaryUiTest.hideChild())
    ...
    await page.evaluate(() => globalThis.capillaryUiTest.destroyParentTwice())
    expect(await page.evaluate(() => globalThis.capillaryUiTest.lifecycleCounts.destroy)).toBe(1)
