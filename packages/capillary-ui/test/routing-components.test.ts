...
import type {ComponentProps, CapillaryUiChild} from '../src/index.js'
...
describe('routed Capillary UI components', () => {
    ...
    test('outlet applies eager and lazy retention without tab semantics', async () => {
        for (const policy of ['eager', 'lazy'] as const) {
            ...
            assert.deepEqual(lifecycle, policy === 'eager'
                ...
                : ['initialize first'])
            active.set('second')
            assert.equal(requiredQuery('cap-routeoutlet > div:not([hidden])').textContent, 'second')
            active.set('first')
