...
import type {ComponentProps, CapillaryUiChild, LivePropContract} from '../src/Components/component.js'
...
describe('DOM patching', () => {
    test('marks every renderer-created element and preserves the marker', () => {
        class MarkerProbe extends Component {
            render() {
                return h('main', {'data-cap': false},
                    h('button', {dataset: {capillaryUi: null}}, 'Save'),
                )
            }
        }
        ...
        assert.equal(main.hasAttribute('data-cap'), true)
        assert.equal(button.hasAttribute('data-cap'), true)
    })
...
describe('vnode and JSX forms', () => {
    ...
    test('supports stateless function components', () => {
        ...
        function Greeting({name, children}: GreetingProps): CapillaryUiChild {
        ...
        assert.equal(document.body.innerHTML, '<p data-cap="">Hello Ada!</p>')
    })
    ...
    test('rejects invalid children and duplicate sibling keys clearly', () => {
        ...
        assert.throws(() => Invalid.new(), /Invalid Capillary UI child/)
...
describe('Capillary template bindings', () => {
    test('renders emitter children as fine-grained binding ranges', () => {
        const content = new Emitter<CapillaryUiChild>('first')
        let renders = 0
