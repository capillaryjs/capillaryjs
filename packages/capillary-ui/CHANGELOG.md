# Capillary UI changelog

All notable changes to `@capillaryjs/capillary-ui` are documented here. The format
...
- Renamed the package from `@sylwellsoftware/fray` to
  `@capillaryjs/capillary-ui`, public `Fray*`/`fray*` symbols to
  `CapillaryUi*`/`capillaryUi*`, and supported `fray-*` custom elements and
