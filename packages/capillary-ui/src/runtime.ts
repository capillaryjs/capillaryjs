...
export interface CapillaryUiRuntimeOptions {
    ...
    /** Static Capillary UI message overrides and formatting locale for this runtime. */
    localization?: CapillaryUiLocalizationOptions
}

/** Application scope for structural styles, browser integrations, and localization. */
export class CapillaryUiRuntime {
    ...
    constructor(
        options: CapillaryUiRuntimeOptions = {},
        registry: StyleRegistry = createStyleRegistry(),
    ) {
        if (options == null || typeof options !== 'object' || Array.isArray(options)) {
            throw new TypeError('CapillaryUiRuntime options must be an object')
        }
        ...
        if (!(this.services instanceof ServiceScope)) {
            throw new TypeError('Capillary UI services must be a ServiceScope')
        }
        ...
        if (this.router != null
            ...
            throw new TypeError('Capillary UI router must be a BrowserRouter')
        }
    }

    /** Resolve a fixed one-hyphen public host name for a built-in component. */
    resolveElementName(hostName: string): string {
        assertNamePart(hostName, 'Capillary UI component host name')
        const resolved = `cap-${hostName.replaceAll('-', '')}`
    ...
    registerStyles(dependency: ComponentDependency): this {
        if (dependency == null || typeof dependency.registerStyles !== 'function') {
            throw new TypeError('registerStyles requires a Capillary UI component dependency')
        }
...
export function createCapillaryUiRuntime(options: CapillaryUiRuntimeOptions = {}): CapillaryUiRuntime {
    return new CapillaryUiRuntime(options)
}

/** Runtime used by Component.new(), direct mount(), and the legacy styleRegistry export. */
export const defaultCapillaryUiRuntime = new CapillaryUiRuntime({}, styleRegistry)
...
function assertCustomElementName(value: unknown): asserts value is string {
    assertNamePart(value, 'Capillary UI element name')
    if (!value.includes('-') || RESERVED_CUSTOM_ELEMENT_NAMES.has(value)) {
        throw new TypeError(`Capillary UI element name must be a non-reserved name containing a hyphen: ${value}`)
    }
}
