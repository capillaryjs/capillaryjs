...
export const Fragment = Symbol.for('@capillaryjs/capillary-ui.Fragment')

export type Key = string | number

export interface ComponentProps {
    key?: Key | null
    children?: CapillaryUiChild | readonly CapillaryUiChild[]
    class?: string | null
...
export type CapillaryUiChild =
    ...
    | VNode
    | Component
    | ReadableEmitter<CapillaryUiChild, unknown>
    | readonly CapillaryUiChild[]

type NormalizedChild = string | number | VNode | Component | ReadableEmitter<CapillaryUiChild, unknown>
...
export interface ComponentDependency {
    registerStyles(runtime?: CapillaryUiRuntime, seen?: Set<ComponentDependency>): void
}
...
type CapillaryUiDOMProps<TElement extends HTMLElement> =
...
export interface CapillaryUiHostElementTagNameMap {
    'cap-content': HTMLElement
    'cap-checkshell': HTMLElement
    'cap-selectshell': HTMLElement
    'cap-expander': HTMLElement
    'cap-label': HTMLElement
    'cap-options': HTMLElement
    'cap-error': HTMLElement
    'cap-erroricon': HTMLElement
    'cap-errortext': HTMLElement
    'cap-toolbarcontent': HTMLElement
    'cap-primary': HTMLElement
    'cap-secondary': HTMLElement
    'cap-separator': HTMLElement
    'cap-progress': HTMLElement
    'cap-inverse': HTMLElement
    'cap-presets': HTMLElement
    'cap-draghandle': HTMLElement
    'cap-chart': HTMLElement
    'cap-readout': HTMLElement
    'cap-swatch': HTMLElement
    ...
    'cap-categoryhidecontent': HTMLElement
    'cap-criteriongroups': HTMLElement
    'cap-categorygroupcontent': HTMLElement
    'cap-categories': HTMLElement
    'cap-categoryoption': HTMLElement
    'cap-categoryswatch': HTMLElement
    'cap-infopanel': HTMLElement
    'cap-datepicker': HTMLElement
    'cap-timepicker': HTMLElement
    'cap-datetimepicker': HTMLElement
}

export type CapillaryUiElementTagNameMap = HTMLElementTagNameMap & CapillaryUiHostElementTagNameMap

export type CapillaryUiIntrinsicElements = {
    [TTag in keyof CapillaryUiElementTagNameMap]: CapillaryUiDOMProps<CapillaryUiElementTagNameMap[TTag]>
        & LiveDOMProperties<CapillaryUiElementTagNameMap[TTag]>
        & NativeValueBinding<CapillaryUiElementTagNameMap[TTag]>
        & NativeCheckedBinding<CapillaryUiElementTagNameMap[TTag]>
}
...
export namespace h {
    export namespace JSX {
        ...
        export type IntrinsicElements = CapillaryUiIntrinsicElements
    }
}
...
export class Component<TProps extends ComponentProps = ComponentProps> {
    ...
    static registerStyles(
        ...
        runtime: CapillaryUiRuntime = defaultCapillaryUiRuntime,
    ...
    /** @internal */
    _runtime: CapillaryUiRuntime = defaultCapillaryUiRuntime
    ...
    protected host(props: ComponentProps | null = null, ...children: CapillaryUiChild[]): VNode {
    ...
    _setRuntime(runtime: CapillaryUiRuntime): this {
        if (!(runtime instanceof CapillaryUiRuntime)) {
            throw new TypeError('Component runtime must be a CapillaryUiRuntime')
        }
    ...
    /** Read a Capillary value, fetch state, and error as one tracked render dependency. */
    ...
    /** Read the immutable Capillary UI localization selected for this component's runtime. */
    protected get localization(): CapillaryUiLocalization {
        return this._runtime.localization
    }

    /** Resolve one Capillary UI-authored message with the runtime's English fallback. */
    protected capillaryUiMessage<TKey extends keyof CapillaryUiMessageOverrides>(
        key: TKey,
    ): CapillaryUiMessage<TKey> {
...
interface EmitterRecord extends RecordBase {
    ...
    source: ReadableEmitter<CapillaryUiChild, unknown>
    start: Comment
    end: Comment
    child: RenderRecord
    unsubscribe: () => void
...
function normalizeRoot(value: CapillaryUiChild): NormalizedChild {
...
function normalizeChildren(values: CapillaryUiChild | readonly CapillaryUiChild[]): NormalizedChild[] {
    const normalized: NormalizedChild[] = []
    const visit = (value: CapillaryUiChild): void => {
...
function createRecord(value: NormalizedChild, owner: Component): RenderRecord {
    ...
    if (isReadableEmitter(value)) {
        const start = document.createComment('cap-emitter-start')
        const end = document.createComment('cap-emitter-end')
    ...
    if (!isVNode(value)) {
        throw new TypeError(`Invalid Capillary UI child: ${describeValue(value)}`)
    }

    const {type, key = null, props} = value
    if (type === Fragment) {
        const start = document.createComment('cap-fragment-start')
        const end = document.createComment('cap-fragment-end')
...
function patchChildren(
    ...
    values: CapillaryUiChild | readonly CapillaryUiChild[],
    owner: Component,
    end: Node | null = null,
...
function patchElementProps(record: ElementRecord, nextProps: ComponentProps): void {
    const next = Object.fromEntries(Object.entries(nextProps).filter(([key]) =>
        key !== 'children' && normalizeAttributeName(key) !== CAPILLARY_UI_RENDERER_ATTRIBUTE,
    )) as ComponentProps
...
function isReadableEmitter(value: unknown): value is ReadableEmitter<CapillaryUiChild, unknown> {
