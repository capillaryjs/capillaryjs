...
export class Calendar extends Component<CalendarProps> {
    render(): CapillaryUiChild {
        ...
        const focusedDayId = `cap-calendar-day-${focusedDate}`
        ...
        const rows: CapillaryUiChild[] = []
        for (let row = 0; row < rowCount; row += 1) {
            const cells: CapillaryUiChild[] = []
            for (let col = 0; col < 7; col += 1) {
                ...
                const dayId = `cap-calendar-day-${date}`
        ...
        return (
            <div className="cap-calendar">
                <div className="cap-calendar-header">
                    <button
                        type="button"
                        aria-label={this.capillaryUiMessage('calendarPreviousMonthLabel')}
                        onClick={() => setMonth(-1)}
                    >
                        {'<'}
                    </button>
                    ...
                    <button
                        type="button"
                        aria-label={this.capillaryUiMessage('calendarNextMonthLabel')}
                        onClick={() => setMonth(1)}
                    >
                        {'>'}
                    </button>
                ...
                <table
                    role="grid"
                    aria-label={this.capillaryUiMessage('calendarGridLabel')}
                    tabIndex={-1}
