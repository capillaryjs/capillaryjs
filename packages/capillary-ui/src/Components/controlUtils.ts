...
import type {EmitterOptions, ReadableEmitter} from '@capillaryjs/capillary'

import type {Component, ComponentProps} from './component.js'
import {layoutAllocationClassName} from './layout/layoutTraits.js'
import type {CapillaryUiLayoutParticipantProps} from './layout/layoutTraits.js'
...
export function controlId(prefix: string, provided?: string | number | null): string {
    ...
    return `cap-${prefix}-${nextControlId++}`
}
...
export function layoutParticipantClass(
    props: Pick<ComponentProps, 'class' | 'className'> & CapillaryUiLayoutParticipantProps,
): string {
