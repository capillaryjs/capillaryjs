import {Button, Checkbox, Component, GroupBox, css} from '@capillaryjs/capillary-ui'
...
extends Component<SplitSelectionPanelProps<TItem>> {
    ...
    static css = css`
        ...
        & > cap-groupbox > cap-content > p,
        & > cap-groupbox > cap-content > ol {
            margin: 0;
        }

        & > cap-groupbox > cap-content > p:first-child {
        ...
        & > cap-groupbox > cap-content > p[role="status"][aria-live="polite"][aria-atomic="true"] {
            position: absolute;
            width: 1px;
            height: 1px;
            padding: 0;
    ...
    private focusHandle(criterionKey: string): void {
        const handles = this.dom instanceof Element
            ? this.dom.querySelectorAll<HTMLElement>('[data-split-key] cap-draghandle')
            : []
