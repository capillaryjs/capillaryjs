...
import {GroupBox, OptionGroupHeaderEnd, h} from '@capillaryjs/capillary-ui'
...
test('generated structural CSS uses fixed visualization hosts without theme selectors', async () => {
    ...
    assert.match(css, /cap-blocklabel/)
    assert.doesNotMatch(css, /data-capillary-viz|data-part/)
    assert.doesNotMatch(css, /datacomponentlike|datacomponentshell|coloredlike|coloredinner/)
    assert.doesNotMatch(css, /data-cap-component/)
...
describe('visualization controls', () => {
    test('CollapsibleOptionGroup preserves its header region and inherited feedback', () => {
        ...
        assert.match(
            required('cap-collapsibleoptiongroup legend').textContent ?? '',
            /Severity4 visible/,
        )
        assert.equal(
            required('cap-collapsibleoptiongroup fieldset > div > p').textContent,
            'Options',
        )
        assert.equal(required('cap-collapsibleoptiongroup fieldset')
            .getAttribute('aria-busy'), 'true')
        group.setProps({label: 'Severity', error: new Error('Options unavailable')})
        assert.equal(required('cap-collapsibleoptiongroup cap-error[role="alert"]')
    ...
    test('CategoryHidePanel uses Capillary UI checkboxes and unfiltered live counts', () => {
        assert.match(CategoryHidePanel.css, /flex: 0 0 auto/)
        assert.match(GroupBox.css, /display:\s*flex[^}]*flex-flow:\s*var\(--cap-groupbox-flow, row nowrap\)/)
        assert.match(GroupBox.css, /> cap-header\s*\{[^}]*writing-mode:\s*var\(--cap-groupbox-header-writing, vertical-rl\)[^}]*transform:\s*var\(--cap-groupbox-header-transform, rotate\(180deg\)\)/)
        assert.match(CategoryHidePanel.css, /cap-collapsible-option-group > fieldset > legend small/)
        assert.match(CategoryHidePanel.css, /cap-categoryoption > cap-checkbox > label/)
        assert.doesNotMatch(CategoryHidePanel.css, /cap-check-box/)
        ...
        assert.equal(required('cap-categoryhidepanel').getAttribute('data-cap-component'),
            'category-hide-panel')
        const categoryPanel = required('cap-categoryhidepanel')
        assert.equal(categoryPanel.getAttribute('role'), 'group')
        assert.ok(categoryPanel.querySelector(':scope > cap-header'))
        assert.ok(categoryPanel.querySelector(
            ':scope > cap-content > cap-categoryhidecontent',
        ))
        assert.ok(required('cap-collapsibleoptiongroup'))
        assert.ok(required('cap-categories'))
        const categoryOptions = [...document.querySelectorAll<HTMLElement>('cap-categoryoption')]
        ...
        assert.equal(document.querySelectorAll('cap-categoryswatch').length, 2)
    ...
    test('SplitSelectionPanel applies presets and pointer/keyboard reorder with focus retention', async () => {
        ...
        assert.equal(required('cap-splitselectionpanel').getAttribute('data-cap-component'),
            'split-selection-panel')
        const splitPanel = required('cap-splitselectionpanel')
        assert.equal(splitPanel.getAttribute('role'), null)
        assert.ok(splitPanel.querySelector(':scope > cap-groupbox > cap-header'))
        assert.ok(splitPanel.querySelector(':scope > cap-groupbox > cap-content > ol'))
        assert.equal(required('[data-split-key="state"]').className, '')
        assert.equal(document.querySelector('[data-part="position"]'), null)
        assert.equal(required('cap-draghandle').localName, 'cap-draghandle')
        assert.equal(document.querySelector('cap-draghandle button'), null)
        assert.equal(required('[data-split-key="state"]')
            .lastElementChild?.localName, 'cap-draghandle')
...
describe('BlockGraph', () => {
    test('bakes each category c1/c2/c3 triplet into blocks while themes own chrome visibility', () => {
        ...
        assert.match(BlockGraph.css, /\[role="treeitem"\] > cap-blocklabel\s*\{[^}]*position: absolute/)
        assert.doesNotMatch(BlockGraph.css, /cap-blocklabel\s*\{[^}]*min-height:\s*2\.5rem/)
        assert.match(BlockGraph.css, /grid-template-areas:\s*"label count"/)
        assert.match(BlockGraph.css, /cap-blocklabel > cap-blockname\s*\{[^}]*grid-template-columns:\s*auto minmax\(0, 1fr\)/)
...
describe('LineGraph', () => {
    ...
    test('renders SVG series, generated legend, and pointer/keyboard cursor readout', () => {
        ...
        assert.equal(required('cap-linegraph').getAttribute('data-cap-component'), 'line-graph')
        assert.ok(required('cap-chart'))
        assert.ok(required('cap-swatch'))
        assert.equal(document.querySelectorAll('.seriesline').length, 2)
        assert.equal(document.querySelectorAll('.seriesarea').length, 0)
        assert.match(required('cap-readout').textContent ?? '', /2026-01-05/)

        const chart = required<HTMLElement>('cap-chart')
        Object.defineProperty(chart, 'getBoundingClientRect', {
            configurable: true,
            value: () => ({left: 0, width: 960}),
        })
        chart.dispatchEvent(new MouseEvent('pointermove', {bubbles: true, clientX: 72}))
        assert.match(required('cap-readout').textContent ?? '', /2026-01-01/)
        ...
        assert.match(required('cap-readout').textContent ?? '', /2026-01-04/)
